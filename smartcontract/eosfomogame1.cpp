#include "eosfomogame1.hpp"

void eosfomogame1::transfer(account_name from, account_name to, asset quantity, string memo) {
    if (from == _self || to != _self) {
        return;
    }
    eosio_assert(quantity.symbol == S(4,EOS), "eosfomogame1 only accepts EOS");
...
void eosfomogame1::withdraw(account_name to) {
    ...
    if (vault > 0) {
        ...
        action(
                ...
                N(eosio.token),
                N(transfer),
                make_tuple(_self, to, vault_asset, string("eosfomogame1 withdraw"))
        ).send();
    }
}

void eosfomogame1::create(time_point_sec start) {
    require_auth(_self);
